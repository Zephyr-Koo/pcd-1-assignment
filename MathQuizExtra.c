...
int displayQuizQuestion_Version2(int operation, int level, int nthQuestion); // EXTRA
...
int runQuizGenerator_Version2(int operation, int level); // EXTRA
            ...
            do
            {
                ...
                if (isValidLevel(currentLevel))
                {
                    while (isValidLevel(currentLevel))
                    {
                        ...
                        int  score = runQuizGenerator_Version2(currentOperation, currentLevel);
                        char shouldContinue;
...
int displayQuizQuestion_Version2(int operation, int level, int nthQuestion)
{
    int correctAnswer = (operation == OPERATION_MULTIPLICATION ? 1 : 0); // initialize with identity element

    printf("\nQuestion %d of %d\n", nthQuestion, NO_OF_QUESTION);

    for (int n = 1; n <= level + 1; n++) // generate 2 operands for Beginner, 3 operands for Intermediate and so on
    {
        int randomNumber = rand() % 99 + 1; // 1 - 99
        ...
        {
            correctAnswer += (n == 1 ? randomNumber : -randomNumber); // add the first operand as positive and followed by negative operands
            printf("%d - ", randomNumber);
        }
        else if (operation == OPERATION_MULTIPLICATION)
        {
            correctAnswer *= randomNumber;
            printf("%d x ", randomNumber);
        }
    }

    printf("\b\b = "); // remove ending operator and space from previous output
...
int runQuizGenerator_Version2(int operation, int level)
{
    int noOfCorrectAnswer = 0;

    for (int n = 1; n <= NO_OF_QUESTION; n++)
    {
        int systemAnswer, userAnswer;

        system("cls");
        srand(time(NULL));
        displayQuizMenu(operation, level);

        systemAnswer = displayQuizQuestion_Version2(operation, level, n);
        scanf("%d", &userAnswer);
